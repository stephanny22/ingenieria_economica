...
const SimpleInterestCalculator = () => {
  ...
  // Función para convertir el tiempo a años según la unidad seleccionada
  ...
  const calculate = () => {
    ...
    switch (variableToCalculate) {
      ...
      case 'rate':
        ...
        // Dentro de la función calculate
case 'years':
  I = parseFloat(interest);
  P = parseFloat(principal);
  R = parseFloat(rate) / 100;
  ...
  if (isNaN(I) || isNaN(P) || isNaN(R)) {
    ...
    const timeInYears = convertToYears(N, timeUnit);
    if (isNaN(timeInYears)) {
