...
export const HomePage = () => {
  ...
  return (
    <div className="flex flex-wrap h-screen" style={{ backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed', minHeight: '100vh'}}>
      <div className="w-full h-0 p-5 text-center">
        <span className="block text-3xl font-bold text-cyan-400 mb-3">INTEGRANTES</span>
      </div>
      {members.map((member, index) => (
        <div key={index} className="w-full h-100 sm:w-1/2 md:w-1/3 lg:w-1/4 p-8"> {/* Reduje el padding */}
